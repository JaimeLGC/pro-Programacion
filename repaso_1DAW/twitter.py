...
class User:
    ...
    def login(self, password: str) -> None:
        ...
        self.logged = True if res.fetchone() else False
    ...
    def retweet(self, tweet_id: int) -> Tweet:
        ...
        if not self.logged:
            raise TwitterError(f'User {self.username} is not logged in!')
        if not self.cur.execute(f'SELECT * FROM tweet WHERE id = {tweet_id}').fetchone():
...
class Tweet:
    ...
    def save(self, user: User) -> None:
        ...
        Además actualiza el atributo "id" del objeto a partir de lo que devuelve la inserción.'''
        sql = 'INSERT INTO tweet(content, retweet_from, user_id) VALUES(?, ?, ?)'
        self.cur.execute(sql, (self.content, self.retweet_from, user.id))
        self.con.commit()
        self.id = self.cur.lastrowid
...
class Twitter:
    ...
    def add_user(self, username: str, password: str, bio: str = '') -> User:
        ...
        if re.match(regex, password, re.I):
